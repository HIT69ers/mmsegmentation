...
def reorganize(raw_data_dir: str, out_dir: str):
    ...
    def move_data(data_list, dst_prefix, fname_func):
        ...
        """
        for data_item in data_list:  # data_item: 'RGB/2.jpg'
            data_item = data_item.split('/')
            # new_item = fname_func(data_item)
            shutil.move(
                osp.join(raw_data_dir, data_item[0], data_item[1]),
                osp.join(out_dir, dst_prefix, data_item[1]))

    def process_phase(phase):
        """Process a dataset phase (e.g., 'train' or 'test')."""
        with open(osp.join(raw_data_dir, f'{phase}.txt')) as f:
