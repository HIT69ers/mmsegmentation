...
@MODELS.register_module()
class MyMixVisionTransformer(BaseModule):
    """The backbone of Segformer.
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            x = nlc_to_nchw(x, hw_shape)
            if i in self.out_indices:
                # 将每个stage输出的结果上采样后放入outs中，不影响backbone数据流，所以只需要一次下采样
                self.H, self.W = int(self.H / self.strides[i]), int(self.W / self.strides[i])
                x_ = F.interpolate(x, (self.H, self.W), mode='bilinear', align_corners=False)
                outs.append(x_)
